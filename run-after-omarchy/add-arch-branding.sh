...
get_ref_dir() {
  if [[ -d "${SCRIPT_DIR}/reference-files" ]]; then
    printf "%s\n" "${SCRIPT_DIR}/reference-files"
  elif [[ -d "${HOME}/repos/omarchy-setup/reference-files" ]]; then
    printf "%s\n" "${HOME}/repos/omarchy-setup/reference-files"
  else
    err "Could not find reference-files directory"
    return 1
  fi
}
